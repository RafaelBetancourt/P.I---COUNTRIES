...
router.get('/countries', async (req, res) => {
    ...
    if (nameOrder) {
        ...
        return search.length > 0 ? res.status(201).json(search) : res.status(201).send('Could not order by name ' + nameOrder);
    } else if (popuOrder) {
        let search = await Country.findAll({
            order: [
        ...
        return search.length > 0 ? res.status(201).json(search) : res.status(201).send('Could not order by population ' + popuOrder);
    } else if (continent) {
        let search = await Country.findAll({
            where: {
                continent: {
